 ...
 * FILE:     Starter.c       
 *
 * DESCRIPTION:   Main body of CF demo program. Slightly modified
 *                version of CodeWarrior Starter app.
 *
 * AUTHOR: Trevor Meyer         
 *
 * DATE: 8/10/99           
 ...
 * Description : Check for characters on the serial port and, if found,
 *               pass them to the command interpreter.
 * Params      : none
 * Returns     : nothing
 *--------------------------------------------------------------------------*/
...
{
    Boolean       inserted, changed = false;
    UInt16	  saved_ints;
...
static void AppEventLoop(void)
{
    ...
    do 
    {
        /* note that we set an event timeout, and use the timeout return  */
        /* to check the serial port for user input.                       */
...
static Err AppStart(void)
{
    ...
    if ((appID = DmFindDatabase(0,"CfDemo")) == 0)
    {
    ...
    /* load the CF Library */
    err = SysLibFind(CfLibName, &cf_lib);
    if (err == 0)
        COMPrintf("CF library already loaded\r\n");
    else
    {
        // The library wasn't pre-loaded -- we need to load it ourselves
        if ((err = SysLibLoad(CfLibTypeID, CfLibCreatorID, &cf_lib)) != 0)
        {
            COMPrintf("Could not load the CF library\r\n");
            COMClose();
            return(err);
        }
    }
    
    /* CF Library loaded -- now open it */
    if ((err = CfLibOpen(cf_lib)) != 0)
    {
        COMPrintf("Could not open the Cf library\r\n");
        COMClose();
        return(err);
    }
...
static void AppStop(void)
{
    ...
    /* close and unload the CF library */
    CfLibClose(cf_lib);
    SysLibRemove(cf_lib);
