...
// Local prototypes
extern "C" Err PocketCLibInstall(UInt refNum, SysLibTblEntryPtr entryP);
static Ptr asm PocketCLibDispatchTable(void);


Err PocketCLibInstall(UInt refNum, SysLibTblEntryPtr entryP) {
	// Install pointer to our dispatch table
	entryP->dispatchTblP = (Ptr*)PocketCLibDispatchTable();
