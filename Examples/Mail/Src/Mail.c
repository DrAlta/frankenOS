...
static MemHandle			EditTitleH;								// Title of edit view.
		 ...
		 MemHandle			SyncUIStateH;							// Save save of the HotSync dialog
 ...
 * FUNCTION:    SearchDraw
 *
 * DESCRIPTION: This routine draw the address and subject a a message 
 *              found by the text search routine.
 *
 * PARAMETERS:	 recordP  - a mail record
 *              category - catagory the record is in
 *              r        - draw region
 *
 * RETURNED:	 nothing
 *
 ...
 *
 ***********************************************************************/
static Err AddMessage (MailAddRecordParamsPtr msgParams)
{
	Err 					err;
	UInt16				attr;
	UInt16		 		index;
		...
		*title = 0;
		...
		if (*subject)
			...
			while (true)
				...
				width += TxtCharWidth (ch);		
				if (width >= maxWidth) break;
				}

			if ((width >= maxWidth) && (subject[len] != linefeedChr))
				{
				maxWidth -= TxtCharWidth (chrEllipsis);
 ...
 *
 ***********************************************************************/
static Boolean NewViewSaveEditState (void * table, Int16 row, Int16 /*column*/)
{
	Int16 rowID;
	UInt16 selectStart;
	UInt16 selectEnd;
 ...
 *
 ***********************************************************************/
static Err NewViewGetField (void * table, Int16 row, Int16 /*column*/,
 ...
 *
 ***********************************************************************/
static void NewViewDrawLabel (void * table, Int16 row, Int16 /*column*/, 
	RectanglePtr bounds)
{
	Int16 i;
	Int16 x;
...
UInt32	PilotMain (UInt16 cmd, MemPtr cmdPBP, UInt16 launchFlags)
{
	...
	// add a record to the Mail outbox.
		...
		{
		error = AddMessage ((MailAddRecordParamsPtr)cmdPBP);
		if (error) return (error);
		}
